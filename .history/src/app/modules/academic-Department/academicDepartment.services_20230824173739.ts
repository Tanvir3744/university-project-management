...
const updateDepartment =async (id: string, payload: Partial<IAcademicDepartment>): Promise<IAcademicDepartment> => {
  const result = await AcademicDepartmentModels.findByIdAndUpdate({ _id: id }, payload);
}
