...
const getAllFaculty = async (paginationOptions: IPaginationOptions, filters: IAcademicFacultyFilters): Promise<IGenericResponse<IAcademicFaculty[]>> => {
    ...
    const { page=1, limit=10, sortBy, sortOrder, skip } = pagintationHelpers.calculatePagination(paginationOptions);
...
const academicFacultiesUpdate = async (id: string, payload: Partial<IAcademicFaculty>): Promise<IAcademicFaculty | null> => {
...
const deleteAcademicFaculty = async (id: string) => {
    const result = await AcademicFacultyModels.findByIdAndDelete(id);
    return result;
}
